...
const CONFIG = {
  '/activites': {
    table: 'd_lieu', id: 'id_lieu', title: 'nom',
    fields: ['nom', 'pays', 'ville', 'quartier', 'description', 'adresse', 'lien_photo'],
    label: 'Lieu / visite / musée',
  },
  '/carnet-gastronomique': {
    table: 'd_plat', id: 'id_plat', title: 'nom_plat',
    fields: ['nom_plat', 'nom_restaurant', 'adresse_restaurant', 'ville', 'pays', 'prix', 'lien_photo', 'notes'],
    label: 'Gastronomie',
  },
  '/voyage-commun': {
    table: 's_voyage_commun', id: 'id_post', title: 'titre',
    fields: ['titre', 'pays', 'ville', 'description'],
    label: 'Le Comptoir Voyage',
  },
  '/playlist': {
    table: 's_musique', id: 'id_musique', title: 'titre',
    fields: ['titre', 'artiste', 'pays', 'lien_spotify', 'lien_youtube', 'lien_apple_music', 'lien_deezer'],
    label: 'Playlist du voyage',
  },
  '/depenses': {
    table: 's_depense', id: 'id_depense', title: 'intitule',
    fields: ['intitule', 'montant', 'devise', 'categorie', 'date_depense'],
    label: 'Dépense',
  },
}

const LABELS = {
  nom: 'Nom', pays: 'Pays', ville: 'Ville', quartier: 'Quartier', description: 'Description', adresse: 'Adresse',
  lien_photo: 'Photo', nom_plat: 'Nom du plat', nom_restaurant: 'Restaurant', adresse_restaurant: 'Adresse du restaurant',
  prix: 'Prix', notes: 'Notes', titre: 'Titre', artiste: 'Artiste', lien_spotify: 'Spotify', lien_youtube: 'YouTube',
  lien_apple_music: 'Apple Music', lien_deezer: 'Deezer', intitule: 'Intitulé', montant: 'Montant', devise: 'Devise',
    ...
    <div className="fixed inset-0 z-[1200] bg-navy/45 flex items-center justify-center p-4" onClick={() => !saving && onClose()}>
      <div className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex flex-col gap-3">
          {target.config.fields.map((field) => (
            <label key={field} className="text-xs text-navy/65">{LABELS[field] || field}
              {field === 'description' || field === 'notes' ? <textarea value={draft[field] ?? ''} onChange={(e) => setDraft((d) => ({ ...d, [field]: e.target.value }))} rows={4} className="mt-1 w-full px-3 py-2.5 border border-navy/15 rounded-lg text-sm text-navy" />
                : <input type={field === 'date_depense' ? 'date' : field === 'montant' ? 'number' : 'text'} value={draft[field] ?? ''} onChange={(e) => setDraft((d) => ({ ...d, [field]: e.target.value }))} className="mt-1 w-full px-3 py-2.5 border border-navy/15 rounded-lg text-sm text-navy" />}
            </label>
          ))}
        </div>
        {error && <p className="text-xs text-red-500 mt-3">{error}</p>}
        <button type="button" onClick={save} disabled={saving} className="btn-primary w-full justify-center mt-5 disabled:opacity-60">{saving ? 'Enregistrement…' : 'Enregistrer'}</button>
      </div>
    </div>, document.body
  )
}
...
export default function ContentEditEnhancer({ children }) {
  ...
  useEffect(() => {
    if (!config || !user) { setRecords([]); return }
    supabase.from(config.table).select('*').order('created_at', { ascending: false }).then(({ data }) => setRecords(data || []))
  }, [config, user])
  ...
  return <>
    {children}
    {mounted.map((item) => createPortal(<EditButton onClick={() => setTarget(item)} />, item.host, item.key))}
    {target && <EditModal target={target} onClose={() => setTarget(null)} onSaved={saved} />}
  </>
}
