    ...
    {FIELDS.map((field) => (
      <label key={field} className="text-xs text-navy/65">
        ...
        {field === 'pays' ? <div className="mt-1"><PaysAutocomplete label="" placeholder="Pays" value={draft[field] ?? ''} onChange={(value) => setDraft((d) => ({ ...d, [field]: value }))} /></div>
          ...
          : <input type={field === 'lien_resa' ? 'url' : 'text'} inputMode={field === 'prix_indicatif' ? 'text' : undefined} value={draft[field] ?? ''} onChange={(e) => setDraft((d) => ({ ...d, [field]: e.target.value }))} className="mt-1 w-full px-3 py-2.5 border border-navy/15 rounded-lg text-sm" />}
      </label>
    ))}
  </div>
}
